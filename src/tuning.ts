...
export const TUNING = {
  ...
  cameraXMargin: 1.8, // 鏡頭橫向最多跟到離建築線這麼近（公尺）：人貼牆時鏡頭停在外面（橫式鏡頭 2.4 m 高在招牌下方，1.8 就夠）
  ...
  cameraPortrait: {
    ...
    fov: 82,
    lookAhead: 17,
    // 直式鏡頭 4.6 m 高，跟直立招牌同一層（招牌底 ≥ 3.8 m、最多凸出牆面 1.85 m），
    // 離建築線要留更多才不會被招牌擋住視線；2.8 是算過所有款式都閃得開的值
    xMargin: 2.8,
  },
  ...
  // ── 車頭燈光暈（從背後來的同向車，車頭前方地面一片光暈）──
  // 車還在畫面外時光暈就先滑進畫面底部，玩家看得到「哪條車道亮起來、亮得多快」就知道往哪閃
  headlight: {
    enabled: true, // 總開關（false = 完全不生光暈；量效能用）
    length: 12, // 光暈往車頭前方延伸幾公尺
    showAheadUntil: 12, // 車開到玩家前方超過這麼多公尺就關掉光暈（看得到車了；大面積透明平面很吃顯示卡填色率）
    width: 3.2, // 光暈最寬處幾公尺（約一條車道）
    opacity: 0.55, // 亮度（0～1）
    color: 0xfff1c0, // 光色（暖白）
    bikes: true, // 人行道腳踏車（右側從背後來）也給一片小的
    bikeScale: 0.45, // 腳踏車光暈的長寬比例
  },
  ...
  parking: {
    ...
    types: {
      ...
      car: {
        ...
        occupancy: 0.8,
        // 車寬 1.8 < 人行道 3.4：車貼著建築那側停，靠馬路那側留 1.6 的縫，行人擠得過去；完全封死是機車格的工作
        blockSize: { x: 1.8, y: 1.3, z: 4.4 },
        edgeGap: 0, // 車身（碰撞框）離停車格靠建築那側邊緣多遠：0 = 緊貼商店路緣
      },
    },
  },
  ...
  intersection: {
    ...
    everyMin: 50, // 之後每隔幾公尺一個路口（隨機取 min~max；關卡表可用 intersectionEveryMin/Max 覆寫）
    everyMax: 100,
    roadDepth: 15, // 橫向小路的縱深（公尺）——縱向斑馬線要走多長就調這個
    spawnZ: 80, // 路口生成在前方多遠（生成點要藏在霧裡；測試時可暫調 40 就近看）
    turnChance: 0.99, // 靠人行道車道的迎面車在路口右轉的機率（卡車不轉）
