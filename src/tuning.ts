...
export const TUNING = {
  ...
  cameraOrbit: {
    ratio: 1.0, // 跟多少：1 = 完全繞到人物背後、0.3 = 只跟三成（側身時還看得到車流）、0 = 關掉（永遠看前方）
    damp: 4, // 繞的平滑度（越大越快）
    // 按鍵要不要跟著鏡頭：true = ↑ 永遠是「往畫面前方走」、→ 是「往畫面右邊走」（鏡頭轉了按鍵跟著轉）；
    // false = ↑ 永遠是沿著馬路前進、→ 永遠是往馬路右邊，不管鏡頭轉到哪
    keysFollowCamera: true,
  },
